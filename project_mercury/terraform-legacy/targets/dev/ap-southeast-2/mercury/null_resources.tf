...
resource "null_resource" "wait_for_stable_mercury_service" {
  ...
  provisioner "local-exec" {
    command = "echo disabled"
    #command = "AWS_ACCESS_KEY_ID=$(jq -r '.Credentials .AccessKeyId' ./.creds)  AWS_SECRET_ACCESS_KEY=$(jq -r '.Credentials .SecretAccessKey' ./.creds) AWS_SESSION_TOKEN=$(jq -r '.Credentials .SessionToken' ./.creds) AWS_DEFAULT_REGION='ap-southeast-2' aws ecs wait services-stable --region 'ap-southeast-2' --cluster ${var.deploy_to_cluster} --services ${module.app.mercury_service_name}"
  }
}
