module "app" {
  ...
  cloudwatch_log_group               = "/${var.ENV}/us-east-1/${var.PROJECT}"
  desired_amount                     = "${var.desired_amount}"

  security_group_ids                 = [
    "${lookup(data.terraform_remote_state.network.mercury_app, "us-east-1")}",
    "${lookup(data.terraform_remote_state.network.mercury_app_from_spr_network, "us-east-1")}"
  ]
  subnet_ids                         = [
    "${lookup(data.terraform_remote_state.network.mercury_app_private_subnet, "us-east-1c")}",
    "${lookup(data.terraform_remote_state.network.mercury_app_private_subnet, "us-east-1b")}"
  ]
  cpu                                = "${var.mercury_cpu}"
  ...
  sns_alarm_arn                      = [
    "arn:aws:sns:us-east-1:${var.qaint_account_id}:ALARMS"
  ]
  ...
  account_id                         = "${var.qaint_account_id}"
