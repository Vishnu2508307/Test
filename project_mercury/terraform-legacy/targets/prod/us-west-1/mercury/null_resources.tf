...
resource "null_resource" "wait_for_stable_mercury_service" {
  ...
  provisioner "local-exec" {
    command = "echo disabled"
    #command = "AWS_ACCESS_KEY_ID=$(jq -r '.Credentials .AccessKeyId' ./.creds)  AWS_SECRET_ACCESS_KEY=$(jq -r '.Credentials .SecretAccessKey' ./.creds) AWS_SESSION_TOKEN=$(jq -r '.Credentials .SessionToken' ./.creds) AWS_DEFAULT_REGION='us-west-1' aws ecs wait services-stable --region 'us-west-1' --cluster ${var.deploy_to_cluster} --services ${module.app.mercury_service_name}"
  }
}
