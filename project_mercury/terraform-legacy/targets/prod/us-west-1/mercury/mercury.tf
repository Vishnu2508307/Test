module "app" {
  ...
  cloudwatch_log_group               = "/${var.ENV}/us-west-1/${var.PROJECT}"
  desired_amount                     = "${var.desired_amount}"

  security_group_ids                 = [
    "${lookup(data.terraform_remote_state.network.mercury_app, "us-west-1")}",
    "${lookup(data.terraform_remote_state.network.mercury_app_from_spr_network, "us-west-1")}"
  ]
  subnet_ids                         = [
    "${lookup(data.terraform_remote_state.network.mercury_app_private_subnet, "us-west-1a")}",
    "${lookup(data.terraform_remote_state.network.mercury_app_private_subnet, "us-west-1b")}"
  ]
  cpu                                = "${var.mercury_cpu}"
  ...
  sns_alarm_arn                      = [
    "arn:aws:sns:us-west-1:${var.prod_account_id}:ALARMS"
  ]
