module "ingestion" {
  ...
  subnet_ids             = [
    "${lookup(data.terraform_remote_state.network.ingestion_private_subnet, "us-west-1a")}",
    "${lookup(data.terraform_remote_state.network.ingestion_private_subnet, "us-west-1b")}",
  ]
  cpu                    = "${var.ingestion_cpu}"
  memory                 = "${var.ingestion_memory}"
  sns_alarm_arn          = ["arn:aws:sns:${data.aws_region.current.name}:${var.prod_account_id}:ALARMS"]
