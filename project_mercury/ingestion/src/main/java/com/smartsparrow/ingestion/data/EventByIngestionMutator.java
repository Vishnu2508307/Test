...
class EventByIngestionMutator extends SimpleTableMutator<IngestionEvent> {

    @Override
    public String getUpsertQuery(IngestionEvent mutation) {
        return "INSERT INTO ingestion.event_by_ingestion (" +
                "  ingestion_id" +
                ", event_id" +
                ", project_id" +
                ", event_type" +
    ...
    public void bindUpsert(BoundStatement stmt, IngestionEvent mutation) {
        stmt.setUUID(0, mutation.getIngestionId());
    ...
    public String getDeleteQuery(final IngestionEvent mutation) {
        return "DELETE FROM ingestion.event_by_ingestion" +
                " WHERE ingestion_id = ?";
    }
