Feature: Restart activity with Graph pathways
  ...
  Scenario: User should be able to restart activity with graph pathway after completion of root activity
    ...
    And the evaluation result has
      ...
      | interactiveCompleted | true            |
      | fired_scenarios      | 1               |
    When "Alice" fetches the progress for the root activity "ACTIVITY_TWO" for cohort "cohort" deployment "DEPLOYMENT_ONE"
    Then the root activity has completion
      | confidence | 0.5 |
      | value      | 0.5 |
    ...
    Then the root activity has completion
      | confidence | 1 |
      | value      | 1 |
    When "Alice" restarts the "ACTIVITY_ONE" activity for "DEPLOYMENT_ONE" deployment
    When "Alice" fetches the progress for the root activity "ACTIVITY_ONE" for cohort "cohort" deployment "DEPLOYMENT_ONE"
    Then the root activity has completion
      | confidence | 0 |
      | value      | 0 |

  Scenario: User should be able to restart activity with graph pathway in middle
    And "Alice" has created a scenario "SCENARIO_ONE" for the "INTERACTIVE_ONE" interactive with "INTERACTIVE_COMPLETE_AND_GO_TO" action for
      | elementId       | ACTIVITY_TWO    |
      | elementType     | ACTIVITY        |
      | expected        | 8               |
