...
public class AllowProjectOwner implements AuthorizationPredicate<ProjectMessage> {

    private static final Logger log = LoggerFactory.getLogger(AllowProjectOwner.class);


    private final ProjectPermissionService projectPermissionService;

    @Inject
    public AllowProjectOwner(final ProjectPermissionService projectPermissionService) {
    ...
    public boolean test(final AuthenticationContext authenticationContext, final ProjectMessage projectMessage) {
        ...
        if (projectMessage.getProjectId() != null) {
            if (account != null) {
                ...
                return permissionLevel != null && permissionLevel.isEqualOrHigherThan(PermissionLevel.OWNER);
            }
        }
