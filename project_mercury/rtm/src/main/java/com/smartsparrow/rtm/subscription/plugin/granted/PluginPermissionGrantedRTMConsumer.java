...
public class PluginPermissionGrantedRTMConsumer implements RTMConsumer<PluginPermissionGrantedRTMConsumable> {

    MercuryLogger logger = MercuryLoggerFactory.getLogger(PluginPermissionGrantedRTMConsumer.class);
    ...
    @Inject
    public PluginPermissionGrantedRTMConsumer(final AccountService accountService,
     ...
     * @param pluginPermissionGrantedRTMConsumable the produced consumable
     */
    @SuppressWarnings("Duplicates")
    @Override
    public void accept(final RTMClient rtmClient,
                       final PluginPermissionGrantedRTMConsumable pluginPermissionGrantedRTMConsumable) {
        final RTMClientContext producingRTMClientContext = pluginPermissionGrantedRTMConsumable.getRTMClientContext();
        if (!rtmClient.getRtmClientContext().getClientId().equals(producingRTMClientContext.getClientId())) {
            PluginPermissionBroadcastMessage message = pluginPermissionGrantedRTMConsumable.getContent();

            final String broadcastType = pluginPermissionGrantedRTMConsumable.getBroadcastType();
            final UUID subscriptionId = pluginPermissionGrantedRTMConsumable.getSubscriptionId();
