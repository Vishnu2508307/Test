...
class PathwayConfigChangeRTMConsumerTest {

    @InjectMocks
    private PathwayConfigChangeRTMConsumer configChangeRTMConsumer;
    ...
    void setUp() {
        ...
        when(pathwayConfigChangeRTMConsumable.getContent()).thenReturn(message);
        when(pathwayConfigChangeRTMConsumable.getBroadcastType()).thenReturn(broadcastType);
        when(pathwayConfigChangeRTMConsumable.getSubscriptionId()).thenReturn(subscriptionId);
    ...
    void accept_filterOutSameClientProducer() {
        when(pathwayConfigChangeRTMConsumable.getRTMClientContext()).thenReturn(rtmClientContext);

        configChangeRTMConsumer.accept(rtmClient, pathwayConfigChangeRTMConsumable);
    ...
    void accept_writesToWebSocket() {
        ...
        when(pathwayConfigChangeRTMConsumable.getRTMClientContext()).thenReturn(producer);
        ...
        final String expected = "{" +
                ...
                "\"response\":{" +
                    ...
                    "\"elementType\":\"" + PATHWAY + "\"," +
                    "\"config\":\"" + config + "\"," +
                    "\"rtmEvent\":\"PATHWAY_CONFIG_CHANGE\"," +
