...
class ComponentMovedRTMConsumerTest {
    @InjectMocks
    private ComponentMovedRTMConsumer movedRTMConsumer;
    ...
    void setUp() {
        ...
        when(componentMovedRTMConsumable.getContent()).thenReturn(message);
        when(componentMovedRTMConsumable.getBroadcastType()).thenReturn(broadcastType);
        when(componentMovedRTMConsumable.getSubscriptionId()).thenReturn(subscriptionId);
    ...
    void accept_filterOutSameClientProducer() {
        when(componentMovedRTMConsumable.getRTMClientContext()).thenReturn(rtmClientContext);

        movedRTMConsumer.accept(rtmClient, componentMovedRTMConsumable);
    ...
    void accept_writesToWebSocket() {
        ...
        when(componentMovedRTMConsumable.getRTMClientContext()).thenReturn(producer);
