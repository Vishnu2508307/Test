...
class EvaluableSetRTMConsumerTest {

    @InjectMocks
    private EvaluableSetRTMConsumer evaluableSetRTMConsumer;
    ...
    @Mock
    private EvaluableSetBroadcastMessage message;
    ...
    void setUp() {
        ...
        when(evaluableSetRTMConsumable.getContent()).thenReturn(message);
        when(evaluableSetRTMConsumable.getBroadcastType()).thenReturn(broadcastType);
        when(evaluableSetRTMConsumable.getSubscriptionId()).thenReturn(subscriptionId);

        when(message.getElementId()).thenReturn(activityId);
        when(message.getElementType()).thenReturn(ACTIVITY);
        when(message.getEvaluationMode()).thenReturn(DEFAULT);
    }

    @Test
    @DisplayName("It should filter out the same client producer")
    void accept_filterOutSameClientProducer() {
        when(evaluableSetRTMConsumable.getRTMClientContext()).thenReturn(rtmClientContext);

        evaluableSetRTMConsumer.accept(rtmClient, evaluableSetRTMConsumable);
    ...
    void accept_writesToWebSocket() {
        ...
        when(evaluableSetRTMConsumable.getRTMClientContext()).thenReturn(producer);
        ...
        final String expected = "{" +
                ...
                "\"response\":{" +
                    ...
                    "\"rtmEvent\":\"ACTIVITY_EVALUABLE_SET\"," +
                    "\"action\":\"EVALUABLE_SET\"" +
                "}," +
