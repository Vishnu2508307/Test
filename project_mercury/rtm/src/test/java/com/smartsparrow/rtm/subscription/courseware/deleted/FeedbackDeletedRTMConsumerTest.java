...
class FeedbackDeletedRTMConsumerTest {
    @InjectMocks
    private FeedbackDeletedRTMConsumer deletedRTMConsumer;
    ...
    void setUp() {
        ...
        when(feedbackDeletedRTMConsumable.getContent()).thenReturn(message);
        when(feedbackDeletedRTMConsumable.getBroadcastType()).thenReturn(broadcastType);
        when(feedbackDeletedRTMConsumable.getSubscriptionId()).thenReturn(subscriptionId);
    ...
    void accept_filterOutSameClientProducer() {
        when(feedbackDeletedRTMConsumable.getRTMClientContext()).thenReturn(rtmClientContext);

        deletedRTMConsumer.accept(rtmClient, feedbackDeletedRTMConsumable);
    ...
    void accept_writesToWebSocket() {
        ...
        when(feedbackDeletedRTMConsumable.getRTMClientContext()).thenReturn(producer);
