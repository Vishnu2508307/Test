...
public class ItemAssociationByDocumentMaterializer implements TableMaterializer {

    private final PreparedStatementCache stmtCache;

    @Inject
    public ItemAssociationByDocumentMaterializer(PreparedStatementCache stmtCache) {
        this.stmtCache = stmtCache;
    }

    public Statement fetchAssociations(UUID documentId) {
        String BY_ID = "SELECT document_id, " +
                "association_id FROM competency.item_association_by_document " +
                "WHERE document_id = ?";
