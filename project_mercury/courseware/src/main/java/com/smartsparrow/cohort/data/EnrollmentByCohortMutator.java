...
public class EnrollmentByCohortMutator extends SimpleTableMutator<CohortEnrollment> {

    @Override
    public String getUpsertQuery(CohortEnrollment mutation) {
        String INSERT = "INSERT INTO cohort_enrollment.enrollment_by_cohort (" +
                "cohort_id, " +
                "account_id, " +
                "enrollment_date, " +
                "enrolled_at, " +
    ...
    public String getDeleteQuery(CohortEnrollment mutation) {
        return "DELETE FROM cohort_enrollment.enrollment_by_cohort " +
                "WHERE cohort_id = ? " +
                "AND account_id = ?";
    }
    ...
    public void bindDelete(BoundStatement stmt, CohortEnrollment mutation) {
        stmt.bind(mutation.getCohortId(), mutation.getAccountId());
    }
}
