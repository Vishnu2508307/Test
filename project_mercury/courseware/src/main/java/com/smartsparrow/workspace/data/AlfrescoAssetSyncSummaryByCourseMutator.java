...
class AlfrescoAssetSyncSummaryByCourseMutator extends SimpleTableMutator<AlfrescoAssetSyncSummary> {

    @Override
    public Statement upsert(AlfrescoAssetSyncSummary mutation) {

        // @formatter:off
        String QUERY = "INSERT INTO asset.alfresco_sync_summary_by_course (" +
                " course_id" +
                ", reference_id" +
                ", sync_type" +
                ", status" +
    ...
    public void bindUpsert(BoundStatement stmt, AlfrescoAssetSyncSummary mutation) {
        stmt.setUUID(0, mutation.getCourseId());
