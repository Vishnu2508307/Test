...
public class DeploymentLogByStateMaterializer implements TableMaterializer {

    private final PreparedStatementCache stmtCache;

    @Inject
    public DeploymentLogByStateMaterializer(PreparedStatementCache stmtCache) {
        this.stmtCache = stmtCache;
    }

    public Statement fetch(UUID cohortId, UUID deploymentId, UUID changeId, DeploymentStepState state) {
        final String SELECT = "SELECT" +
                ...
                ", element_id" +
                ", element_type" +
                " FROM learner.deployment_log_by_state" +
                " WHERE cohort_id = ?" +
                " AND deployment_id = ?" +
                " AND change_id = ?" +
                " AND state = ?";
        ...
        stmt.bind(cohortId, deploymentId, changeId, Enums.asString(state));
