...
public class CompetencyMetByStudentHistoryMutator extends SimpleTableMutator<CompetencyMet> {

    @Override
    public String getUpsertQuery(CompetencyMet mutation) {
        return "INSERT INTO learner.competency_met_by_student_document_history ( " +
                "student_id, " +
                "document_id, " +
                "document_item_id, " +
                "met_id, " +
    ...
    public String getDeleteQuery(CompetencyMet mutation) {
        return "DELETE FROM learner.competency_met_by_student_document_history WHERE " +
                "student_id = ? AND " +
                "document_id = ? AND " +
                "document_item_id = ? AND " +
                "met_id = ? ";
    }
