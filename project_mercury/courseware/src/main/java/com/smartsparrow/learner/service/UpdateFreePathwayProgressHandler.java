...
/**
 * Process a Progress update for a FREE pathway type
 */
@Singleton
public class UpdateFreePathwayProgressHandler extends UpdateProgressHandler {
    ...
    @Inject
    public UpdateFreePathwayProgressHandler(StudentProgressRTMProducer studentProgressRTMProducer,
    ...
    public void updateProgress(final Exchange exchange) {
        ...
        Mono<FreePathwayProgress> ppm = progressService.findLatestFreePathway(deploymentId, pathwayId, studentId);
        FreePathwayProgress previousProgress = ppm.block();
        if(previousProgress == null) {
            previousProgress = new FreePathwayProgress();
        }
        ...
        // Build the progress
        FreePathwayProgress currentProgress = buildProgress(evaluationEventMessage, attemptId, childProgress, event,
                newCompletionValues, newCompletionConfidence, completion, completedWalkables);

        // persist the progress
        progressService.persist(currentProgress).blockLast();
    ...
    private FreePathwayProgress buildProgress(final EvaluationEventMessage evaluationEventMessage,
    ...
    private FreePathwayProgress buildProgressFromAction(final UpdateCoursewareElementProgressEvent event,
                                                        EvaluationEventMessage evaluationEventMessage,
                                                        final UUID attemptId,
                                                        final Map<UUID, Float> newCompletionValues,
                                                        final Map<UUID, Float> newCompletionConfidence,
                                                        final Completion completion,
    ...
    private FreePathwayProgress buildNormalProgress(final UpdateCoursewareElementProgressEvent event,
                                                    final UUID attemptId,
                                                    final Map<UUID, Float> newCompletionValues,
                                                    final Map<UUID, Float> newCompletionConfidence,
                                                    final Completion completion,
