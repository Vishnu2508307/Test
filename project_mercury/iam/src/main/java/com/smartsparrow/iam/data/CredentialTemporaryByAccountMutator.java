...
class CredentialTemporaryByAccountMutator extends SimpleTableMutator<CredentialTemporary> {

    @Override
    public String getUpsertQuery(CredentialTemporary mutation) {
        // @formatter:off
        return "INSERT INTO iam_global.credential_temporary_by_account ("
                + "  authorization_code"
                + ", type"
                + ", account_id"
                + ") VALUES ( ?, ?, ? )"
    ...
    public String getDeleteQuery(CredentialTemporary mutation) {
        // @formatter:off
        return "DELETE FROM iam_global.credential_temporary_by_account"
                + "  WHERE account_id = ?"
                + "    AND type = ?"
    ...
    public void bindDelete(BoundStatement stmt, CredentialTemporary mutation) {
        stmt.bind(mutation.getAccountId(), mutation.getType().name(), mutation.getAuthorizationCode());
    }
